...
# This class will only prepare the payload for the UE5 model
...
class UE5Model:
    ...
    def set_model(self, model_name: str) -> None:
        """
        Set the model with its name and load the model information. This method will initialize the `self.model_info`, `self.action_map`, and `self.action_str` attributes.
        ...
        self.action_map: dict = {
            k.lower(): v for k, v in self.model_info["actionMap"].items()
        }
        self.action_str: str = " ".join([f"[{key}]," for key in self.action_map.keys()])
        # action_str is a string of the keys in the actionMap dictionary. The keys are enclosed in square brackets.
    ...
    def extract_action(self, str_to_check: str) -> list:
        """
        Check the input string for any action keywords and return a list of values (the expression index) of the actions found in the string.

        Parameters:
            str_to_check (str): The string to check for actions.

        Returns:
            list: A list of values of the actions found in the string. An empty list is returned if no actions are found.
        """
        ...
        while i < len(str_to_check):
            ...
            for key in self.action_map.keys():
                action_tag = f"[{key}]"
                if str_to_check[i : i + len(action_tag)] == action_tag:
    ...
    def ractionve_action_keywords(self, target_str: str) -> str:
        """
        Ractionve the action keywords from the input string and return the cleaned string.

        Parameters:
            str_to_check (str): The string to check for actions.

        Returns:
            str: The cleaned string with the action keywords ractionved.
        """
    ...
    Attributes:
        ...
        action_map (dict): The action map of the Live2D model.
    ...
    def extract_action(self, str_to_check: str) -> list:
        ...
        while i < len(str_to_check):
            ...
            for key in self.action_map.keys():
                action_tag = f"[{key}]"
                if str_to_check[i : i + len(action_tag)] == action_tag:
                    expression_list.append(self.action_map[key])
                    i += len(action_tag) - 1
                    break
            i += 1
